...
## USBXHCI Transfer Family Assessment
...
That pass splits the strongest transfer bodies into two subfamilies.
...
- helper-heavy shared spine:
  ...
  - `0x000049B4`
  - `0x00058B00`
- only `0x00010440` stays substantive on that side
...
## USBXHCI 10440 Helper Branch Assessment
...
That pass resolves the shared helper-heavy transfer body directly.
...
That pass turns the primary controller timing body into a compact direct-branch map.
...
That pass turns the secondary controller timing body into the same direct-branch map format as `0x0001B1F0`.
...
## USBXHCI Controller Core Compare
...
That pass compares the two surviving controller timing bodies directly.
...
That pass maps the strongest surviving transfer-side leaf directly.
...
That pass turns the raw `0x0001B1F0` call sites into bounded timing and branch windows.
...
- `0x0001B1F0` should not be treated as one undifferentiated body anymore
...
## USBXHCI Controller Window Compare
...
That pass compares the bounded controller windows directly instead of comparing only whole bodies.

Observed on this machine:

- output:
  - `out\dev\usbxhci-controller-window-compare.txt`
- top-ranked controller windows:
...
That pass turns the ranked windows into an experiment-ready order with explicit safety bars.

Observed on this machine:

- output:
  - `out\dev\usbxhci-window-intervention-plan.txt`
- first experiment-ready windows:
  - `0x0001B1F0 timer-lifecycle-and-side-context`
  - `0x0001B1F0 time-sampling-and-debug-gate`
  - `0x0003634C wrapper-and-time-anchor`
...
- the earlier watchdog result is now an explicit guardrail against repeating unsafe inline-delay designs
...
That pass narrows the first host-side experiment to one bounded window inside the primary controller body.
...
- the branch now has a bounded first experiment target inside 0x0001B1F0
...
That pass turns the `0x0001B1F0` experiment spec into exact first probe candidates.
...
That pass fixes the event row format and rollup buckets for the first observation run.
...
That pass defines the fixed hot-path event record and the parser-side correlated objects.
...
- the harness now has a default `HostEmitter` binary:
...
- current `Kernel` vs default `HostEmitter` comparison is `Comparable` on this machine:
  ...
  - malformed rows preserved at zero
  - cadence-matched chains preserved
