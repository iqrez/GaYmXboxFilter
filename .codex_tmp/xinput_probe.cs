...
public static class XInputProbe {
  ...
  public static void Main() {
    for (uint i = 0; i < 4; i++) {
      XINPUT_STATE s; uint r = XInputGetState(i, out s);
      Console.WriteLine(string.Format("pad={0} result={1} pkt={2} LX={3} Btn={4}", i, r, r==0 ? s.dwPacketNumber : 0, r==0 ? s.Gamepad.sThumbLX : 0, r==0 ? s.Gamepad.wButtons : 0));
    }
  }
}
