...
The spike now also includes a final intervention summary:

- `scripts\capture-usbxhci-final-intervention-summary.ps1`

Current final intervention summary on this machine:
...
The spike now also includes a `0x0003634C` window map:

- `scripts\capture-usbxhci-3634c-window-map.ps1`

Current `0x0003634C` window map on this machine:
...
The spike now also includes a `0x00015D30` window map:

- `scripts\capture-usbxhci-15d30-window-map.ps1`

Current `0x00015D30` window map on this machine:
...
- `thunk-terminal-edge` is terminal noise rather than a serious study target

The spike now also includes a cross-family window comparison:

- `scripts\capture-usbxhci-cross-family-window-compare.ps1`

Current cross-family window comparison on this machine:

- output:
  - `out\dev\usbxhci-cross-family-window-compare.txt`
- top-ranked cross-family windows are now:
  - `0x0001B1F0 timer-lifecycle-and-side-context`
  - `0x0001B1F0 time-sampling-and-debug-gate`
  - `0x0003634C wrapper-and-time-anchor`
  - `0x0003634C delayed-sleep-and-trace-side`
...
- the strongest transfer-side window is now explicitly positioned as the first contrast/fallback window after the controller family

The spike now also includes a window modality matrix:

- `scripts\capture-usbxhci-window-modality-matrix.ps1`

Current window modality matrix on this machine:

- output:
  - `out\dev\usbxhci-window-modality-matrix.txt`
- top modality anchors are now:
  - `0x0001B1F0 timer-lifecycle-and-side-context`
  - `0x0001B1F0 time-sampling-and-debug-gate`
  - `0x0003634C wrapper-and-time-anchor`
  - `0x0003634C delayed-sleep-and-trace-side`
...
The spike now also includes a focused `0x0001B1F0` observation prototype:

- `scripts\capture-usbxhci-1b1f0-observation-prototype.ps1`

Current `0x0001B1F0` observation prototype on this machine:
...
The spike now also includes a focused `0x0001B1F0` capture pipeline:

- `scripts\capture-usbxhci-1b1f0-capture-pipeline.ps1`

Current `0x0001B1F0` capture pipeline on this machine:

- output:
  - `out\dev\usbxhci-1b1f0-capture-pipeline.txt`
- main artifacts are now:
...
- the branch explicitly prefers buffered numeric event rows over live formatting on hot paths
- the prototype is now concrete enough to implement without rethinking capture flow

The spike now also includes a focused `0x0001B1F0` parser and rollup spec:

- `scripts\capture-usbxhci-1b1f0-parser-rollup-spec.ps1`

Current `0x0001B1F0` parser and rollup spec on this machine:
...
- `HostEmitter` now also supports an `import` backend for externally captured binary event streams
- that import path keeps the same session manifest, rollup, and comparison flow while allowing future host-side capture to be replayed without changing the harness
- the lower control path now also exposes a route-state query so the tools can distinguish:
  - no bound filter context
  - bound filter not in D0
  - observation-capable dev build
- the `0x0001B1F0` harness now preflights live hardware presence for `Kernel` and `HostEmitter Adapter` sources before capture begins
