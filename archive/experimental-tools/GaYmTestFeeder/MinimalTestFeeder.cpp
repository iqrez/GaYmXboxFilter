/*
 * MinimalTestFeeder.cpp  –  GaYmFilter user-mode test tool
 *
 * Opens \\.\GaYmFilter, enables override, and injects GAYM_REPORT structs.
 * Two modes:
 ...
 *   cl /EHsc /W4 /D_CRT_SECURE_NO_WARNINGS /Fe:MinimalTestFeeder.exe MinimalTestFeeder.cpp /I..\GaYmFilter /link user32.lib
 *
 * Run (elevated):
...
/* Pull the real definitions shared with the kernel driver */
#include "ioctl.h"
...
static void RunScripted(HANDLE hDevice) {
    ...
    for (int i = 0; i < stepCount; i++) {
        ...
        while ((elapsed = GetTickCount() - start) < (DWORD)s.durationMs) {
            if (!SendIoctl(hDevice, IOCTL_GAYM_INJECT_REPORT,
                           &s.report, sizeof(s.report), NULL, 0, NULL)) {
...
static void RunKeyboard(HANDLE hDevice) {
    ...
    while (true) {
        ...
        if (!SendIoctl(hDevice, IOCTL_GAYM_INJECT_REPORT,
                       &report, sizeof(report), NULL, 0, NULL)) {
            printf("WARNING: Inject failed (err %lu)\n", GetLastError());
            Sleep(100);
            continue;
        }
...
static BOOL WINAPI CtrlHandler(DWORD dwType) {
    if (dwType == CTRL_C_EVENT || dwType == CTRL_CLOSE_EVENT) {
        ...
        if (g_hDevice != INVALID_HANDLE_VALUE) {
            SendIoctl(g_hDevice, IOCTL_GAYM_OVERRIDE_OFF, NULL, 0, NULL, 0, NULL);
...
int main(int argc, char* argv[]) {
    ...
    printf("GaYmFilter Test Feeder v2.0\n");
    printf("Mode: %s\n", scripted ? "SCRIPTED (30s auto-test)" : "KEYBOARD (manual)");
    printf("sizeof(GAYM_REPORT) = %zu bytes\n\n", sizeof(GAYM_REPORT));

    /* Open device via control device object (CDO) — bypasses xinputhid */
    HANDLE hDevice = INVALID_HANDLE_VALUE;
    {
        /* Try CDO first: \\.\GaYmFilterCtl */
        printf("Trying CDO path: \\\\.\\GaYmFilterCtl\n");
        hDevice = CreateFileW(
            L"\\\\.\\GaYmFilterCtl",
        ...
        if (hDevice != INVALID_HANDLE_VALUE) {
            printf("Opened via CDO (direct path, bypasses xinputhid).\n");
        } else {
            DWORD cdoErr = GetLastError();
            printf("CDO not available (err %lu), trying device interface GUID...\n", cdoErr);

            /* Fallback: device interface GUID (works when filter is upper) */
            HDEVINFO devInfo = SetupDiGetClassDevsW(
                &GUID_DEVINTERFACE_GAYM_FILTER, NULL, NULL,
                DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);

            if (devInfo == INVALID_HANDLE_VALUE) {
                printf("ERROR: SetupDiGetClassDevs failed (error %lu)\n", GetLastError());
                return 1;
            }
            ...
            if (!SetupDiEnumDeviceInterfaces(devInfo, NULL,
                    &GUID_DEVINTERFACE_GAYM_FILTER, 0, &ifData)) {
                printf("ERROR: No GaYmFilter device found (error %lu)\n", GetLastError());
                printf("  -> Is the driver loaded? Check: sc query GaYmFilter\n");
            ...
            SP_DEVICE_INTERFACE_DETAIL_DATA_W* detail =
                (SP_DEVICE_INTERFACE_DETAIL_DATA_W*)malloc(reqSize);
            detail->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA_W);

            if (!SetupDiGetDeviceInterfaceDetailW(devInfo, &ifData, detail, reqSize, NULL, NULL)) {
                printf("ERROR: Cannot get device path (error %lu)\n", GetLastError());
                free(detail);
                SetupDiDestroyDeviceInfoList(devInfo);
                return 1;
            }
            ...
            if (hDevice == INVALID_HANDLE_VALUE) {
                ...
                if (err == 5)  printf("  -> Access denied. Run as Administrator.\n");
                if (err == 32) printf("  -> Device busy. Another instance may be running.\n");
                return 1;
            }
        }
    }
    ...
    DWORD returned = 0;
    if (SendIoctl(hDevice, IOCTL_GAYM_QUERY_DEVICE, NULL, 0, &info, sizeof(info), &returned)) {
    ...
    /* Enable override */
    if (!SendIoctl(hDevice, IOCTL_GAYM_OVERRIDE_ON, NULL, 0, NULL, 0, NULL)) {
    ...
    /* Clean shutdown */
    printf("\nDisabling override...\n");
    SendIoctl(hDevice, IOCTL_GAYM_OVERRIDE_OFF, NULL, 0, NULL, 0, NULL);
    CloseHandle(hDevice);
