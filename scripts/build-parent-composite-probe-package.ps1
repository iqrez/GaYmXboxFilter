...
$probeStageRoot = Join-Path $stageRoot 'polling-composite-probe'
$probePackageRoot = Join-Path $probeStageRoot 'package'
$probeInfTemplate = Join-Path $repoRoot 'GaYmFilter\GaYmCompositeProbe.inf'
$probeInf = Join-Path $probePackageRoot 'GaYmCompositeProbe.inf'
$probeCat = Join-Path $probePackageRoot 'GaYmCompositeProbe.cat'
...
if ($LASTEXITCODE -ne 0) {
    throw 'Inf2Cat failed for the composite parent probe package.'
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'signtool failed for the composite parent probe package.'
}
...
Write-Host "Composite parent probe package: $probePackageRoot"
