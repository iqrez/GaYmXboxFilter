...
#define GAYM_XBOX_USB_HID_LEGACY_REPORT_SIZE  16u
#define GAYM_XBOX_USB_HID_NATIVE_REPORT_SIZE  18u
#define GAYM_XBOX_USB_HID_TRAILER             0x83u
...
/* ═══════════════════════════════════════════════════════════════════
 * Xbox - HID child report translator (PID 0x02FF)
 *
 * The current live lower-path packet is an 18-byte native report:
 *   [0]     Report ID (0 on wire for this child)
 *   [1-2]   LX         uint16 LE, center 32768
 *   [3-4]   LY         uint16 LE, center 32768
 *   [5-6]   RX         uint16 LE, center 32768
 *   [7-8]   RY         uint16 LE, center 32768
 *   [9-10]  Trigger word (observed LT word, 0..1023)
 *   [11-12] Reserved / unknown
 *   [13]    Buttons 1-8 bitmap (A/B/X/Y/LB/RB/Back/Start)
 *   [14]    Buttons 9-16 low nibble (L3/R3/Guide/Misc)
 *   [15]    Hat switch (0 neutral, 1-8 directions)
 *   [16]    Reserved / unknown
 *   [17]    Constant trailer (0x83)
 *
 * Some historical builds used a 16-byte packet. Preserve a fallback path
 * for that older layout until every lower-path consumer is confirmed on the
 * 18-byte report.
 * ═══════════════════════════════════════════════════════════════════ */
    ...
    _Inout_ PUCHAR           Seq)
{
    ...
    triggerWord = XboxTriggerU8ToWord(G->TriggerLeft);

    if (Sz >= GAYM_XBOX_USB_HID_NATIVE_REPORT_SIZE) {
        RtlZeroMemory(N, GAYM_XBOX_USB_HID_NATIVE_REPORT_SIZE);
        ...
        N[9] = (UCHAR)(triggerWord & 0xFF);
        N[10] = (UCHAR)(triggerWord >> 8);
        N[13] = G->Buttons[0];
        N[14] = (UCHAR)(G->Buttons[1] & 0x0F);
        N[15] = (G->DPad == GAYM_DPAD_NEUTRAL) ? 0u : (UCHAR)(G->DPad + 1);
        N[17] = GAYM_XBOX_USB_HID_TRAILER;

        *Written = GAYM_XBOX_USB_HID_NATIVE_REPORT_SIZE;
        return STATUS_SUCCESS;
    }

    if (Sz < GAYM_XBOX_USB_HID_LEGACY_REPORT_SIZE) {
        return STATUS_BUFFER_TOO_SMALL;
    }

    RtlZeroMemory(N, GAYM_XBOX_USB_HID_LEGACY_REPORT_SIZE);
    ...
    *Written = GAYM_XBOX_USB_HID_LEGACY_REPORT_SIZE;
    return STATUS_SUCCESS;
}
    ...
    _Out_ PULONG CaptureFlags)
{
    ...
    if (NativeBufferSize < GAYM_XBOX_USB_HID_LEGACY_REPORT_SIZE) {
        return STATUS_BUFFER_TOO_SMALL;
    }

    ResetParsedReport(GenericReport);
    if (NativeBufferSize >= GAYM_XBOX_USB_HID_NATIVE_REPORT_SIZE) {
        if (NativeReport[17] != GAYM_XBOX_USB_HID_TRAILER) {
        ...
        *CaptureFlags = GAYM_CAPTURE_FLAG_VALID |
        ...
        triggerWord = ReadLe16(&NativeReport[9]);
        GenericReport->TriggerLeft = XboxTriggerWordToU8(triggerWord);
        GenericReport->TriggerRight = 0;

        buttonsLow = NativeReport[13];
        if (buttonsLow & GAYM_BTN_A)     GenericReport->Buttons[0] |= GAYM_BTN_A;
        if (buttonsLow & GAYM_BTN_B)     GenericReport->Buttons[0] |= GAYM_BTN_B;
        if (buttonsLow & GAYM_BTN_X)     GenericReport->Buttons[0] |= GAYM_BTN_X;
        if (buttonsLow & GAYM_BTN_Y)     GenericReport->Buttons[0] |= GAYM_BTN_Y;
        if (buttonsLow & GAYM_BTN_LB)    GenericReport->Buttons[0] |= GAYM_BTN_LB;
        if (buttonsLow & GAYM_BTN_RB)    GenericReport->Buttons[0] |= GAYM_BTN_RB;
        if (buttonsLow & GAYM_BTN_BACK)  GenericReport->Buttons[0] |= GAYM_BTN_BACK;
        if (buttonsLow & GAYM_BTN_START) GenericReport->Buttons[0] |= GAYM_BTN_START;

        buttonsHigh = NativeReport[14] & 0x0F;
        if (buttonsHigh & GAYM_BTN_LSTICK) GenericReport->Buttons[1] |= GAYM_BTN_LSTICK;
        if (buttonsHigh & GAYM_BTN_RSTICK) GenericReport->Buttons[1] |= GAYM_BTN_RSTICK;
        if (buttonsHigh & GAYM_BTN_GUIDE)  GenericReport->Buttons[1] |= GAYM_BTN_GUIDE;
        if (buttonsHigh & GAYM_BTN_MISC)   GenericReport->Buttons[1] |= GAYM_BTN_MISC;
    ...
    *CaptureFlags = GAYM_CAPTURE_FLAG_VALID |
    ...
    buttonsHigh = NativeReport[12] & 0x0F;
    if (buttonsHigh & GAYM_BTN_LSTICK) GenericReport->Buttons[1] |= GAYM_BTN_LSTICK;
    if (buttonsHigh & GAYM_BTN_RSTICK) GenericReport->Buttons[1] |= GAYM_BTN_RSTICK;
    if (buttonsHigh & GAYM_BTN_GUIDE)  GenericReport->Buttons[1] |= GAYM_BTN_GUIDE;
    if (buttonsHigh & GAYM_BTN_MISC)   GenericReport->Buttons[1] |= GAYM_BTN_MISC;
...
static const GAYM_DEVICE_DESCRIPTOR g_DeviceTable[] =
{
    ...
    { 0x045E, 0x02FF, GAYM_DEVICE_XBOX_ONE,    "Xbox Controller (XInput HID)", 0x00, GAYM_XBOX_USB_HID_NATIVE_REPORT_SIZE, TranslateXboxUsbHidReport, ParseXboxUsbHidReport, GAYM_XBOX_XINPUT_OUTPUT_SELECTOR, GAYM_XBOX_XINPUT_OUTPUT_PACKET_LENGTH, GAYM_INPUT_CAP_XBOX_USB_GAMEPAD, GAYM_OUTPUT_CAP_XBOX_GAMEPAD,      TranslateXboxXInputOutputState, ParseXboxXInputOutputReport },
